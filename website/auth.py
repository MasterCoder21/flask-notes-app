...
def update_account():
    if request.method == 'POST':
      # First require original email and password
        email = request.form.get('origemail')
        password = request.form.get('origpassword')
        user = User.query.filter_by(email=email).first()
        if user:
            if check_password_hash(user.password, password):
                logout_user()
                ...
                if user:
                    ...
                    db.session.add(new_user)
                    db.session.commit()
                    flash('Account details updated, please login again to continue.', category='success')
                    return redirect(url_for('auth.login'))
